...
class PlainReader {
public:
    PlainReader(byte_t* cursor) : cursor_(cursor), offset_(0) {}
    ...
    T Read(field_id_t field_id) {
        constexpr bool is_string = is_same<T, string>::value;
        field_id_t actual_field_id = *reinterpret_cast<field_id_t *>(cursor_ + offset_);
        if (field_id != actual_field_id){
            if constexpr (is_string) {
                return "";
            } else {
        ...
        offset_ += sizeof(field_id_t);
        if constexpr (is_string) {
            auto size = cursor_[offset_++];
            char buffer[size];
            memcpy(buffer, cursor_ + offset_, size);
            offset_ += sizeof(char) * size;
            string result(buffer, size);
            return result;
        } else {
            T* ptr = reinterpret_cast<T *>(cursor_ + offset_);
    ...
    template <typename T> T ReadEncoded(field_id_t field_id) {
        ...
        offset_ += sizeof(field_id_t);
        byte_t bytes[16];
        memcpy(bytes, cursor_ + offset_, 16);
        T val;
        size_t size;
        if (is_unsigned<T>::value) {
            size = unsigned_decode(bytes, reinterpret_cast<uint64_t *>(&val));
